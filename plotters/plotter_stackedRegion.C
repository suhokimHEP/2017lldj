  ...
  for(unsigned int j=0; j<variables.size(); ++j){
   ...
   for(unsigned int tt=0; tt<loopEnd; ++tt){
     ...
     */
     ...
     std::cout<< "It works till here 1.5" <<std::endl; 
     h_ST = (TH1F*)h_STbar_t->Clone("ST");
     // h_ST->Add(h_STbar_t);
     // h_ST->Add(h_ST_t);
     // h_ST->Add(h_STbar_tW);
     // h_ST->Add(h_ST_tW);
     ...
     std::cout<< "It works till here 1.7" <<std::endl; 


     h_bkgtotal= (TH1F*)h_altDY->Clone("bkgtotal");
     std::cout<< "It works till here 1.75" <<std::endl; 
      //h_bkgtotal->Add(h_GJets ) ;
      h_bkgtotal->Add(h_ST    ) ;
      h_bkgtotal->Add(h_ZH    ) ;
      h_bkgtotal->Add(h_altVV    ) ;
      h_bkgtotal->Add(h_altTT    ) ;
      h_bkgtotal->Add(h_VG    ) ;
     std::cout<< "It works till here 1.75" <<std::endl; 
     ...
     // h_other->Add(h_ZH    ) ;
     // h_other->Add(h_GJets ) ;
     std::cout<< "It works till here 1.8" <<std::endl; 
     ...
     if( region.Contains("TwoEle") ){
        h_Data = (TH1F*)h_Data_DoubleEG_B->Clone("Data");
        //h_Data->Add( h_Data_DoubleEG_C )     ; 
        //h_Data->Add( h_Data_DoubleEG_D )     ; 
        //h_Data->Add( h_Data_DoubleEG_E )     ; 
        //h_Data->Add( h_Data_DoubleEG_F )     ; 
    ...
    h_Data->Scale(3);	
     ...
     /*if( region.Contains("TwoMu") ){
        h_Data = (TH1F*)h_Data_DoubleMu_B->Clone("Data");
     ...
     /*if( region.Contains("EleMu") ){
        h_Data = (TH1F*)h_Data_MuonEG_B->Clone("Data");
     ...
     */
     ...
     //Float_t  int_Data_DoubleMu_B          = h_Data_DoubleMu_B                 ->Integral(0,-1);                     
     ...
     Float_t  int_Data_DoubleEG_B          = h_Data_DoubleEG_B                 ->Integral(0,-1);                 
...
//     }
     ...
     bgstack->GetXaxis()->SetTitle("CaloJet"+varname + "    "+description);
     if(!drawData){
     ...
     if(HIP){
      extra2->DrawTextNDC(0.51,0.91,"Eras B");
	 ...
	 h_ratio->Divide(h_bkgtotal);
       ...
       h_ratio->GetXaxis()->SetTitle("		CaloJet srced "+(TString)h_Data->GetTitle()+description);
